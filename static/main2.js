...
function show_current_working_directory(children_info) {
    ...
    children_info.map(x => {
        const childDetails = document.createElement('li');
        childDetails.className = "list-group-item";
        childDetails.textContent = x["basename"];

        if(x["is_dir"]) {
            childDetails.style.color = "#2389F6";
            childDetails.style.cursor = "pointer";

            childDetails.onclick = () => {
                current_working_directory = x["full_seg"];
                load_current_working_directory();
            };
        }

        dirView.appendChild(childDetails);
    });
}
